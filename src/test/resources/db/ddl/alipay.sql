...
CREATE TABLE `pdl_alipay_info` (
  ...
  COMMENT '注册时间',
  `is_real_name`           VARCHAR(20)                  DEFAULT NULL
  COMMENT '是否实名认证',
  `is_protected`           VARCHAR(50)                  DEFAULT NULL
  COMMENT '是否会员保障',
  `balance`                DECIMAL(30, 6)               DEFAULT NULL
  COMMENT '账户余额',
  `income`                 DECIMAL(30, 6)               DEFAULT NULL
  COMMENT '余额宝总收入',
  `amount`                 DECIMAL(30, 6)               DEFAULT NULL
  COMMENT '余额宝总金额',
  `total_amount`           DECIMAL(30, 6)               DEFAULT NULL
  COMMENT '花呗总额度',
  `available_amount`       DECIMAL(30, 6)               DEFAULT NULL
  COMMENT '花呗可用额度',
  `need_to_pay_next_mouth` DECIMAL(30, 6)               DEFAULT NULL
  COMMENT '花呗下期需还',
...
CREATE TABLE `pdl_alipay_trade_record_info` (
  ...
  COMMENT '交易状态',
  `trade_detail_url` VARCHAR(255)                 DEFAULT NULL
  COMMENT '支付宝交易详情URL',
  `create_time`      DATETIME            NOT NULL DEFAULT CURRENT_TIMESTAMP
  COMMENT '创建时间',
  `update_time`      DATETIME            NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
...
CREATE TABLE `pdl_alipay_bank_card` (
  ...
  `user_id`        BIGINT(20)          NOT NULL
  COMMENT '用户 ID',
  `card_last_num`  VARCHAR(4)                   DEFAULT NULL
  COMMENT '银行卡后4位',
  `open_status`    VARCHAR(50)                  DEFAULT NULL
  COMMENT '快捷支付开通状态',
  ...
  COMMENT '银行预留手机号(中间4位带掩码)',
  `active_date`    DATETIME                     DEFAULT NULL
  COMMENT '快捷支付开通日期, 例如: 2013-02-02',
  `show_user_name` VARCHAR(50)                  DEFAULT NULL
  COMMENT '银行卡持卡人姓名',
  `create_time`    DATETIME            NOT NULL DEFAULT CURRENT_TIMESTAMP
  COMMENT '创建时间',
  `update_time`    DATETIME            NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
...
CREATE TABLE `pdl_alipay_charge_account` (
  ...
  `user_id`         BIGINT(20)          NOT NULL
  COMMENT '用户 ID',
  `charge_item`     VARCHAR(50)                  DEFAULT NULL
  COMMENT '缴费项目',
  ...
  COMMENT '缴费号码',
  `charge_reminder` VARCHAR(20)                  DEFAULT NULL
  COMMENT '缴费提醒',
  `create_time`     DATETIME            NOT NULL DEFAULT CURRENT_TIMESTAMP
  COMMENT '创建时间',
  `update_time`     DATETIME            NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
